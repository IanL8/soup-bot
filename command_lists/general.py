...
class CommandList(_commands.CommandList):
    ...
    @_commands.command("roll", desc="Gets a random number [default 100].")
    ...
    @_commands.command("opgg-lookup", desc="Looks up a given league player on op.gg.")
    ...
    @_commands.command("avatar", desc="Fetches a user's pfp by @ing them or writing their name. By default will return your avatar.")
    ...
    @_commands.command("movie-add", desc="Adds a movie to the movie list.")
    ...
    @_commands.command("movie-remove", desc="Removes a movie from the movie list.")
    ...
    @_commands.command("movies", desc="Lists all movies.")
    async def movie_list(self, context):
        ...
        if total_pages > 1:
            ...
            await context.send_message("", embed=embeds[0], view=view)
