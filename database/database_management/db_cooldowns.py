...
def fortune(user) -> str:
    ...
    user_ids = _soup_sql.query(conn, "SELECT uid FROM UserCooldowns WHERE tid='fortune';").values
    if user.id in user_ids:
        last_usage_time = _soup_sql.query(conn, "SELECT start_time FROM UserCooldowns WHERE uid=?;", (user.id,)).values[0]
    else:
        _soup_sql.query(conn, "INSERT INTO UserCooldowns (tid, uid) VALUES ('fortune', ?);", (user.id,))
        last_usage_time = 0

    time_since_last_use = _time.time() - last_usage_time
    if time_since_last_use < cooldown:
        conn.close()
        return f"{_duration_to_string(cooldown - time_since_last_use)} until next fortune redeem."

    _soup_sql.query(conn, "UPDATE UserCooldowns SET start_time=? WHERE tid='fortune' AND uid=?;", (int(_time.time()), user.id))
